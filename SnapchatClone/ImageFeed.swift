            ...
            if let posts = snapshot.value as? [String:AnyObject] {
                user.getReadPostIDs(completion: { (ids) in
                    for postKey in posts.keys {
                        ...
                        let objectBhai = posts[postKey]
                        var userBhai = ""
                        var pathBhai = ""
                        var threadBhai = ""
                        var dateBhai = ""
                        
                        userBhai = objectBhai!.value(forKey: firUsernameNode) as! String
                        pathBhai = objectBhai!.value(forKey: firImagePathNode) as! String
                        threadBhai = objectBhai!.value(forKey: firThreadNode) as! String
                        dateBhai = objectBhai!.value(forKey: firDateNode) as! String
                        let read = ids.contains(postKey)
                        
                        let pp = Post(id: postKey, username: userBhai, postImagePath: pathBhai, thread: threadBhai, dateString: dateBhai, read: read)
                        postArray.append(pp)
